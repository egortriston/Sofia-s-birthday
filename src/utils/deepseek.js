        ...
        const response = await fetch(DEEPSEEK_API_URL, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'Ты Софья — милая и веселая девушка из мира Звёздных Войн. Обращайся к пользователю на "ты". Ты даёшь только КОСВЕННЫЕ подсказки: наводящие вопросы, намёки, контекст, ассоциации. ЗАПРЕЩЕНО: называть правильный ответ, писать цифры/буквы ответа, давать прямые формулировки типа "ответ — это X", "нужно написать Y". Цель — подтолкнуть к размышлению, а не выдать ответ. Будь естественной и в характере Софьи, лаконично.'
                    },
                    {
                        role: 'user',
                        content: `Вопрос: "${question}"\n\nПравильный ответ (только для тебя, не раскрывай его): "${correctAnswer}"${previousAnswerText}\n\nДай одну КОСВЕННУЮ подсказку. Можно: задать наводящий вопрос, напомнить контекст или связанную тему, намекнуть на категорию (без названия ответа), предложить подумать над формулировкой вопроса. НЕ пиши правильный ответ, цифры, первые буквы ответа и прямые подсказки вроде "нужно написать ...". Пиши как живой человек, не как инструкция.`
                    }
                ],
                temperature: 0.9,
                max_tokens: 200
